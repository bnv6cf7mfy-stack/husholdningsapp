...
3. **Single finance dashboard page** (`/finance`) with client-side sub-tabs (Dashboard / Inntekt-kostnad / Konto, see `src/features/finance/components/finance-dashboard.tsx`) instead of separate routes, to keep the v1.0 surface small while still separating concerns in the UI. The data/service layer already separates these concerns, so splitting into real routes later is additive.
...
- `finance_accounts` — likely liquid accounts (checking/buffer/savings/other), payment-enabled flag, draw priority, minimum balance, optional owner (`household_members.id`) or `NULL` for "Felles" (accounts can still be joint/shared).
...
- The forecast chart (month/year/multi-year toggle) is a lightweight inline SVG chart, not a full charting library — deliberately, to avoid introducing a new dependency for v1.0. Month/year selection outside the default (first) period is fetched on demand via `getFinanceChartSelectionAction` → `getFinanceChartSelection` in `queries.ts`; the "multi-year" view and the default month/year are preloaded with `getFinanceOverview`. "Akkumulert" (line) shows running balance/cumulative net contribution; "Per periode" (bar) shows the isolated net for that day/month/year, to spot which periods are cash-negative. Drilldown to underlying occurrences is not yet implemented.
...
- "Rediger" on an existing cash flow (`updateFinanceCashFlowAction`) rewrites all fields (type, name, amount, owner, category, recurrence, valid-to, regulering) from a chosen effective date forward, closing the previous version's `valid_to` the day before; it does not yet support "kun denne forekomsten" (a single occurrence override without versioning the whole series).
- Every cash flow (income/expense) entry **must** have an owner (`owner_member_id` is required in `createFinanceCashFlowSchema`/`updateFinanceCashFlowSchema` — no "Felles" option), defaulting to the household member currently registering it. Accounts are unaffected and may still be `NULL`/"Felles".
- The chart's "Vis for" member filter is a *cumulative net cash flow* view per `owner_member_id` (income minus expense, running total from the forecast start), not a real per-person account balance — accounts are not attributed per member in v1.0, only cash flows are. "Alle (husholdningen samlet)" is the only view backed by real account balances/draw-priority simulation.
- Deleting an account or cash flow is a soft delete (`is_active = false`, and for cash flows also closing `valid_to` at "yesterday"); historical balance snapshots and occurrences are preserved for audit purposes, consistent with the "never hard-delete history" principle.
