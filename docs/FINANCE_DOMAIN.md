...
- UI is a single dashboard (accounts, cash flows, forecast summary) without the full month/year/multi-year chart visualization from the spec; drilldown and charting are a follow-up.
- Editing an existing cash flow only supports "this and future occurrences" (`reviseFinanceCashFlowAction`); "only this occurrence" and full rename/retype of an existing series are not yet exposed in the UI.
