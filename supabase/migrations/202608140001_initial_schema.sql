...
alter table public.ingredients
  ...
  foreign key (default_shopping_category_id)
...
create trigger set_households_updated_at before update on public.households
for each row execute procedure public.set_updated_at();
create trigger set_household_members_updated_at before update on public.household_members
for each row execute procedure public.set_updated_at();
create trigger set_children_updated_at before update on public.children
for each row execute procedure public.set_updated_at();
create trigger set_child_measurements_updated_at before update on public.child_measurements
for each row execute procedure public.set_updated_at();
create trigger set_child_quotes_updated_at before update on public.child_quotes
for each row execute procedure public.set_updated_at();
create trigger set_child_notes_updated_at before update on public.child_notes
for each row execute procedure public.set_updated_at();
create trigger set_child_milestones_updated_at before update on public.child_milestones
for each row execute procedure public.set_updated_at();
create trigger set_shopping_categories_updated_at before update on public.shopping_categories
for each row execute procedure public.set_updated_at();
...
for each row execute procedure public.set_updated_at();
create trigger set_childcare_assignments_updated_at before update on public.childcare_assignments
for each row execute procedure public.set_updated_at();
...
-- Profiles: each user can access only own profile.
...
for update using (public.has_household_role(id, array['owner']))
...
create policy "household_members_manage_owner" on public.household_members
for all using (public.has_household_role(household_id, array['owner']))
with check (public.has_household_role(household_id, array['owner']));

-- Generic household member policy helper for domain tables.
create policy "children_select_member" on public.children
for select using (public.is_household_member(household_id));
create policy "children_modify_adult" on public.children
for all using (public.has_household_role(household_id, array['owner', 'adult']))
with check (public.has_household_role(household_id, array['owner', 'adult']));

create policy "child_measurements_member" on public.child_measurements
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "child_quotes_member" on public.child_quotes
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "child_notes_member" on public.child_notes
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "child_milestones_member" on public.child_milestones
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "shopping_categories_member" on public.shopping_categories
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));
...
create policy "calendar_event_children_member" on public.calendar_event_children
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "childcare_assignments_member" on public.childcare_assignments
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));
...
create policy "recipes_member" on public.recipes
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "recipe_ingredients_member" on public.recipe_ingredients
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));

create policy "pantry_items_member" on public.pantry_items
for all using (public.is_household_member(household_id))
with check (public.is_household_member(household_id));
