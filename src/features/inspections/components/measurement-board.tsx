...
import { attachMeasurementPhotoAction, createInspectionMeasurementAction, createMeasurementPhotoUploadAction, updateInspectionMeasurementAction } from "@/features/inspections/measurement-actions";
...
export function MeasurementBoard({ data }: { data: MeasurementData | null }) {
  ...
  useEffect(() => {
    const nextRequiredMeasurement = room?.measurements.find((measurement) =>
      measurement.lengthCm === null && measurement.widthCm === null && measurement.heightCm === null && measurement.depthCm === null
    );
    if (!nextRequiredMeasurement) return;
    setEditingMeasurementId(nextRequiredMeasurement.id);
    setForm({
      name: nextRequiredMeasurement.name,
      length: "",
      width: "",
      height: "",
      depth: "",
      note: nextRequiredMeasurement.note ?? ""
    });
  }, [room]);
  ...
  const saveMeasurement = async () => {
    ...
    setStatus("saving");
    const payload = { roomId: room.id, name: form.name, lengthCm: toNumberOrNull(form.length), widthCm: toNumberOrNull(form.width), heightCm: toNumberOrNull(form.height), depthCm: toNumberOrNull(form.depth), note: form.note };
    const isEditing = Boolean(editingMeasurementId);
    const result = isEditing ? await updateInspectionMeasurementAction({ ...payload, measurementId: editingMeasurementId }) : await createInspectionMeasurementAction(payload);
    if (!result.ok) { setStatus("error"); setError("Målingen kunne ikke lagres. Kontroller at verdiene er større enn null."); return; }
    if (photo) {
      const measurementId = editingMeasurementId ?? (!isEditing && "measurementId" in result ? result.measurementId : null);
      if (!measurementId) { setStatus("error"); setError("Målingen er lagret, men bildet kunne ikke knyttes til den."); return; }
      ...
      if (uploadError || !(await attachMeasurementPhotoAction({ measurementId, storagePath: upload.storagePath, contentType: photo.type, fileSizeBytes: photo.size })).ok) { setStatus("error"); setError("Målingen er lagret, men bildet kunne ikke lagres."); return; }
    }
    setForm({ name: "", length: "", width: "", height: "", depth: "", note: "" }); setEditingMeasurementId(null); setPhoto(null); if (fileInputRef.current) fileInputRef.current.value = ""; setStatus("idle"); startTransition(() => router.refresh());
  };
