...
import { FinanceInputTab } from "./finance-input-tab";
import { FinanceSettingsTab } from "./finance-settings-tab";
import { FinanceAdminTab } from "./finance-admin-tab";
...
const tabs = [
  ...
  { key: "admin", label: "Administrasjon" }
] as const;
...
export function FinanceDashboard({ overview }: { overview: FinanceOverview }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {activeTab === "input" ? (
        <FinanceInputTab overview={overview} pending={pending} runAction={runAction} setError={setError} />
      ) : null}
      {activeTab === "settings" ? (
        <FinanceSettingsTab overview={overview} pending={pending} runAction={runAction} setError={setError} />
      ) : null}
      {activeTab === "admin" ? (
        <FinanceAdminTab overview={overview} pending={pending} runAction={runAction} setError={setError} />
      ) : null}
    </div>
  );
}
