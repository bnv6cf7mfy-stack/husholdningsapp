...
import { createBallerudDocumentUploadAction, deleteBallerudDocumentAction, registerBallerudDocumentAction } from "@/features/ballerud/document-actions";
...
export function DocumentArchive({ documents }: { documents: BallerudDocument[] }) {
  ...
  const deleteDocument = (document: BallerudDocument) => {
    if (!window.confirm(`Slette "${document.title}" permanent?`)) return;
    setDeletingDocumentId(document.id);
    startTransition(async () => {
      const result = await deleteBallerudDocumentAction(document.id);
      setDeletingDocumentId(null);
      if (!result.ok) {
        setError("Dokumentet kunne ikke slettes. Prov igjen.");
        return;
      }
      router.refresh();
    });
